<div align="center">

[![Discontinued — check aro-renew](https://img.shields.io/badge/DISCONTINUED-check_aro--renew-C62828?style=for-the-badge&labelColor=C62828)](https://github.com/AlekdSANS/aro-renewal)

**This version is archived. Active development continues in [aro-renew](https://github.com/AlekdSANS/aro-renewal).**

</div>

# Project ARO

Project ARO is a small passion project created as a hobby and out of personal interest. It was made for an ARG and is designed as an interactive desktop-style web experience.

The project focuses on building a fictional operating-system-like interface with folders, apps, files, videos, login systems, passwords, and hidden interactions for players to discover.

## Status

This repository is discontinued and preserved as an archive of the original implementation. The project now continues in [aro-renew](https://github.com/AlekdSANS/aro-renewal).

Completed core versions:

- Version 0.1: Basic HTML/CSS interface and desktop
- Version 0.2: SCSS support, JavaScript setup with jQuery, folder and application opener scripts
- Version 0.3: One-time intro sequence, videos and files inside folders, scripts for opening media
- Version 0.4: Draggable folders, desktop selection box, working ID application

Final version in this repository:

- Version 0.5: Application systems and code structure polish

## Features

- Desktop-style interface
- Folder and application opening scripts
- One-time intro sequence
- Draggable folders
- Desktop selection box
- Files and videos hidden inside folders
- Working ID application
- ARG-focused interaction design

## Original Roadmap

This roadmap is retained for historical context and has been superseded by [aro-renew](https://github.com/AlekdSANS/aro-renewal).

### Version 0.5

- Make all applications work and open:
  - Menu
  - Finished menu
  - Internet
  - Social media
  - Workspace with logs
  - Security app
- Polish the code structure

### Version 0.6

- Add settings
- Add user profile login scripts
- Add passwords for files and folders
- Add a search bar cheat code that reveals a draggable list of all passwords
- Add resizable folders

### Version 0.7

- Rewrite and remake everything in React
- Explore an inventory system

## Notes

This project is designed for one specific screen resolution and is not planned to be adapted for other resolutions.

## Tech Stack

- HTML
- CSS
- SCSS
- JavaScript
- jQuery

## Purpose

Project ARO is a personal hobby project made for learning, experimentation, and ARG storytelling.
